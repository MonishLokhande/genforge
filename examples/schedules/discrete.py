...
from forge.core.interfaces import DiscreteSchedule
...
class AbsorbingDiffusion(_D3PM):
    ...
    def reverse_probs(self, xt, t, s, x0_logits) -> torch.Tensor:
        ...
        # SUBS at INFERENCE. Under absorbing corruption x₀ is never the mask token, so p_θ(x₀=mask)≡0
        # and the posterior must place p_θ over DATA tokens only. This is not optional bookkeeping: the
        # training loss zeroes the mask logit (mdlm.py) so that logit receives NO gradient — leaving an
        # UNTRAINED value that would otherwise leak into sampling through the ᾱ_s·p_θ(mask) term and
        # arbitrarily inflate the stay-masked mass. Exclude the mask logit BEFORE the softmax (mask →
        # −inf): softmax's internal max-subtraction makes this numerically stable, whereas renormalizing
        # by (1−p_θ(mask)) after the fact catastrophically cancels in fp32 when the mask logit is large.
        masked_logits = x0_logits.clone()
        masked_logits[..., m] = float("-inf")
        p_x0 = torch.softmax(masked_logits, dim=-1)                # (…, V); p_x0[…, m] == 0 exactly

        unnorm = (ab_s - ab_t) * p_x0                              # un-mask-to-data mass (over data only)
        unnorm[..., m] = 1.0 - ab_s                               # stay-masked mass (ᾱ_s·p_θ(mask)=0)
