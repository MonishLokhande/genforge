...
class D3PM(Method):
    # NOT a NELBO. This ships only the auxiliary cross-entropy term of the paper's hybrid objective
    # L_λ = L_vb + λ·L_CE (Austin et al. Eq. 5), sampled at uniform t and UNWEIGHTED — there is no
    # per-t weighting, so it is not a bound on log p(x) and must not be converted to bits/perplexity.
    # (MDLM sets nelbo=True legitimately: its rate(t) weighting IS the continuous-time NELBO.)
    # Consequence: the `perplexity` metric now fails loudly on d3pm instead of reporting a non-bound.
    nelbo = False
