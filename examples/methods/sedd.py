...
class SEDD(Method):
    def __init__(self, schedule, space, t_eps: float = 1e-3):
        ...
        if self.mask_index is None:
            # The closed-form bounded score entropy below is ABSORBING-graph-specific (it assumes a
            # single mask token as the stationary state). On a non-absorbing space it would silently
            # compute a wrong objective — genforge's defect class is silent-wrong-output, so fail loud
            # (mirrors MDLM's guard).
            raise ValueError(
                "SEDD implements the absorbing-graph score entropy; it requires an absorbing/masking "
                "space (`space.mask_index` set). For a uniform/general discrete space use d3pm."
            )
